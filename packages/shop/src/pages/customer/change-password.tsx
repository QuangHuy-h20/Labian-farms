...
const schema: yup.SchemaOf<ChangePasswordInput> = yup.object().shape({
  ...
  confirmPassword: yup.string().required("Bạn chưa xác nhận mật khẩu mới").default("")
...
const ChangePassword = () => {
  ...
  const { handleSubmit, register, formState: { errors, isSubmitting }, setError } = useForm<ChangePasswordInput>({
  ...
  const onSubmit = async (values: ChangePasswordInput) => {
    const response = await changePassword({
      variables: { changePasswordInput: values }, onCompleted: (data) => {
        if (data.changePassword.success) toast.success(data.changePassword.message)
      },
    ...
    if (changePasswordErrors) changePasswordErrors.map(({ field, message }) => { setError(field as FieldErrorType, { message }) })
  }

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="w-full p-10">
      <PasswordInput name="oldPassword" label="Mật khẩu cũ" {...register("oldPassword")} placeholder="Nhập mật khẩu cũ" error={errors?.oldPassword?.message!} />
      ...
      <PasswordInput name="confirmPassword" label="Mật khẩu cũ" {...register("confirmPassword")} placeholder="Xác nhận mật khẩu" error={errors?.confirmPassword?.message!} />
      <div className="flex justify-end w-full">
        <Button className='px-6' type="submit" loading={isSubmitting} disabled={isSubmitting} size="medium" variant="normal">Xác nhận</Button>
      </div>
    </form>
  )
}
