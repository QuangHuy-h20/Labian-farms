...
export class TourResolver {
  ...
  async createTour(
    ...
    try {
      ...
      if (existingTour)
        return failureResponse(400, false, "Tên tour đã được sử dụng.");
      ...
      return {
        code: 200,
        success: true,
        message: "Tạo tour tham quan thành công",
        tour: response,
      };
    } catch (error) {
      return failureResponse(
  ...
  @UseMiddleware(checkRole)
  async updateTour(
    @Arg("updateTourInput") updateTourInput: UpdateTourInput,
    @Ctx() { req }: Context,
    ...
    // let list: string[] = [];
    try {
      const { id, name, description, startDate, endDate, slot, status } =
        updateTourInput;
      const existingTour = await Tour.findOne(id);
      if (!existingTour)
        return failureResponse(400, false, "Không tìm thấy tour.");
      ...
      return {
        code: 200,
        success: true,
        message: "Thông tin tour đã được cập nhật thành công.",
        tour: response,
      };
    } catch (error) {
      return failureResponse(
  ...
  async deleteTour(
    ...
    try {
      ...
      if (!existingTour)
        return failureResponse(404, false, "Không tìm thấy tour.");

      //   const folder = `farms/${existingFarm.slug}/tour/${existingTour.slug}`;

      if (req.session.roleId === "executive-admin") {
        if (existingTour.isActive)
          return failureResponse(
            400,
            false,
            "Tour tham quan vẫn còn đang hoạt động. Đóng tour và thử lại."
          );
        await ApplyTour.delete({ tourId: id });
        await Tour.delete({ id });
        await uploadFileBucket
          .file(existingTour.image1.split("/").pop() as string)
          .delete();
      ...
      if (existingTour.isActive)
        return failureResponse(
          400,
          false,
          "Tour tham quan vẫn còn đang hoạt động. Vui lòng liên hệ quản lý để tiến hành xử lý."
        );
      else {
        await ApplyTour.delete({ tourId: id });
        await Tour.delete({ id });
        // new Promise((_) =>
        //   deleteFile(folder, existingTour.image1.split("/").pop() as string)
        // );
        return successResponse(200, true, "Xoá tour tham quan thành công.");
      }
    } catch (error) {
  ...
  async applyTour(
    ...
    return await connection.transaction(async (transactionEntityManager) => {
      ...
      const existingApplyTour = await transactionEntityManager.findOne(
        ApplyTour,
        {
          tourId,
          customerId: userId,
      ...
      if (
        existingApplyTour &&
        existingApplyTour.active !== applyTourStatusValue
      ) {
